;(function () {
	...
	var bonusAnimate = function() {
		var bonus = $('#fh5co-bonus');
		if ( bonus.length > 0 ) {	

			bonus.waypoint( function( direction ) {
										
				if( direction === 'down' && !$(this.element).hasClass('animated') ) {


					var sec = bonus.find('.to-animate').length,
						sec = parseInt((sec * 200) + 400);

					setTimeout(function() {
						bonus.find('.to-animate').each(function( k ) {
							var el = $(this);
					...
					setTimeout(function() {
						bonus.find('.to-animate-2').each(function( k ) {
							var el = $(this);
